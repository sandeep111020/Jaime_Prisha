...
import com.example.jaimeprisha.Adapters.FavoritesAdapter;
...
public class MyOrdersScreen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseRef5 = FirebaseDatabase.getInstance().getReference().child("Users").child(currentuser).child("Orders");
        databaseRef5.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.getChildrenCount() == 0) {
                    ...
                    test.setText("No Items In Your Orders");
                }
     ...
     /*   ValueAnimator animator = ValueAnimator.ofFloat(0f, 1f);
        ...
        FirebaseRecyclerOptions<BookingModel> options =
                new FirebaseRecyclerOptions.Builder<BookingModel>()
                        .setQuery(FirebaseDatabase.getInstance().getReference("Users").child(currentuser).child("Orders"), BookingModel.class)
                        .build();

        adapter = new OrdersAdapter(options,getApplicationContext());
